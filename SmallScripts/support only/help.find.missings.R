...
List2 <- merge(List, PreGTD, by.x=c("Group.1"), by.y=c("merge"), all.x=TRUE)




# give a list of cities with distances to cities in the dataset for a specific counrty
X <- PreGTD[which(PreGTD$country_txt == "nicaragua"),]
...
X <- sub("nicaragua", NA, X)
X <- subset(X, !is.na(X))
X <- as.character(X)
X <- data.frame(X, "nicaragua")
X["lookup"] <- paste(X$X, X$X.nicaragua., sep=", ")
nicaraguaLOC <- geocode(X$lookup, output = c("latlon", "latlona", "more", "all"), messaging = FALSE, sensor = FALSE, override_limit = TRUE)
nicaraguaLOC["CityGTD"] <- X$X
nicaraguaLOC <- subset(nicaraguaLOC, !is.na(lon))
ZZ <- WC.UC.dist[which(WC.UC.dist$country.etc == "nicaragua"),]
ZZZ <- subset(ZZ, select = c("long", "lat", "name")) 
ZZZ$fake=1
nicaraguaLOC$fake=1
Zil <-merge(ZZZ, nicaraguaLOC, by=c("fake"))
...
nicaragua.distances <- merge(Zil.min, Zil, by=c("name", "DistKM"))
nicaragua.distances <- nicaragua.distances[order(nicaragua.distances$DistKM),]
write.csv(nicaragua.distances, file="nicaragua.csv")
rm(X,nicaraguaLOC, ZZ, ZZZ, Zil, Zil.min)
