...
X$X <- 1
List <- aggregate(X$X, by=list(X$merge), FUN=sum)
rm(X)
List <- merge(List, PreGTD, by.x=c("Group.1"), by.y=c("merge"), all.x=TRUE)
List <- List[!duplicated(List$Group.1),]
Missing <- subset(List, select=c("country_txt", "GTD.city", "x"))
Missing["attacks"] <- Missing$x
Missing$x <- NULL
Missing <- Missing[order(-Missing$attacks), ]
rm(List)
X <- subset(Missing, attacks>=20)



X["lookup"] <- paste(X$GTD.city, X$country_txt, sep=", ")
missingLOC <- geocode(X$lookup, output = c("latlon", "latlona", "more", "all"), messaging = FALSE, sensor = FALSE)
...
missing.distances <- merge(Zil.min, Zil, by=c("name", "DistKM"))
missing.distances <- missing.distances[order(missing.distances$DistKM),]
write.csv(missing.distances, file="missing.20ormore.csv")
#rm(X,missingLOC, ZZ, ZZZ, Zil, Zil.min)
